...
mydb = mc.connect(user = 'root',host = 'localhost',passwd = 'kshitij123',database = 'high_score')
...
score = 0
my_cursor.execute("select max(score) from high_score_2")
for j in my_cursor:
    for i in j:
        high_score=i
        print(i)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if login_rect.collidepoint(event.pos):
                login_tup = (user_text,user_text2)
                my_cursor.execute("select username,password from user_data")
                for i in my_cursor:
                    if i == login_tup:
                        welcomescreen = False
                        running = True
                    '''if i[0] != login_tup[0]:
                        back_img2 = pygame.image.load("img3.jpg").convert_alpha()
                    if i[1] != login_tup[1]:
                        back_img2 = pygame.image.load("img4.jpg").convert_alpha()'''
...
while running: 
    ...
    if not game_over:
        ...
        if w < 280 and h < 300: 
            ...
            x -= 2.4
            y -= 0.8
        my_cursor.execute("insert into high_score_2(score,date,time) values("+ str(score)+',curdate(),curtime())')
        mydb.commit()
